...
bool PluginController::InitializeController(void)
{
    ...
    {
        typedef void (*PLUGIN_CREATE)(IFileCounterPlugin **); //定义函数指针类型
        PLUGIN_CREATE createProc;                             //声明函数指针

        IFileCounterPlugin *pPlugin = NULL; //创建一个函数指针
        ...
        if (pluginLib != NULL) //判断是否成功绑定链接
        {
            ...
            createProc = (PLUGIN_CREATE)dlsym(pluginLib, "CreateCounter"); // 获取创建功能函数链接，强转成函数指针类型
 ...
 */
bool PluginController::ProcessRequest(int FunctionID, const char *path)
{
